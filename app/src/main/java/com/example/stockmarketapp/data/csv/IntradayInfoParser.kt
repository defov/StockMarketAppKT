...
class IntradayInfoParser @Inject constructor(): CSVParser<IntradayInfo> {
    ...
    override suspend fun parse(stream: InputStream): List<IntradayInfo> {
        ...
        return withContext(Dispatchers.IO) {
            val rawIntradayInfos = csvReader
                .readAll()
                .drop(1)
                .mapNotNull { line ->
            ...
            var intradayInfos = emptyList<IntradayInfo>()
            (1..10).forEach { i ->
                intradayInfos = rawIntradayInfos.filter {
                    it.date.dayOfMonth == LocalDateTime.now().minusDays(i.toLong()).dayOfMonth
                }
                .sortedBy {
                    it.date.hour
                }
